...
module.exports = {
  async index (req, res) {
    ...
    try {
      if (!helper.isNullOrWhitespace(req.query.listQuery)) {
        ...
        response = await MenuType.findAndCountAll({
          where: filter,
          order: [[prop, order]],
          offset: offset,
          limit: limit
        ...
        response = await MenuType.findAll({
  ...
  async post (req, res) {
    ...
    let response
    try {
      console.log('MenuType -> post', req.body)
      ...
      // }
      console.log('MenuType -> post 2 -------------', temp)
      newMenuType = await MenuType.create(temp)
      // for (const obj of files) {
      //   const attachment = {}
      //   attachment.createdBy = req.user.id
      //   attachment.MenuTypeId = newMenuType.id
      //   attachment.type = 'MenuType'
      ...
      // }
      response = await MenuType.findOne({
        where: { id: newMenuType.id }
      })
      res.send(response)
    } catch (err) {
      console.log(err)
  ...
  async update (req, res) {
    ...
    let temp = null
    try {
      console.log('MenuType -> update', req.body)
  ...
  //   try {
  //     for (var i = 0; i < req.body.length; i++) {
  //       await MenuType.update({ isActive: false }, { where: { id: req.body[i].id } })
  //     }
  //     res.send('success')
  //   } catch (err) {
  //     res.status(500).send({
  ...
  //   try {
  //     for (var i = 0; i < req.body.length; i++) {
  //       await MenuType.update({ isActive: true }, { where: { id: req.body[i].id } })
  //     }
  //     res.send('success')
  //   } catch (err) {
  //     res.status(500).send({
