const {RefOrganization, User} = require('../../models')
...
module.exports = {
  async index (req, res) {
    ...
    try {
      if (req.query.listQuery !== undefined && req.query.listQuery !== null && req.query.listQuery !== undefined) {
        ...
        response = await RefOrganization.findAndCountAll({
          where: filter,
          order: [
            [prop, order]
          ],
        ...
        response = await RefOrganization.findAll({
          where: { isActive: true },
  ...
  async post (req, res) {
    try {
      const exist = await RefOrganization.findAndCountAll({
      ...
      const response = await RefOrganization.create(req.body)
      res.send(response)
    } catch (err) {
      console.log(err)
      res.status(500).send({
  ...
  async put (req, res) {
    try {
      const exist = await RefOrganization.find({
        where: {code: req.body.code}
      })
      ...
      await RefOrganization.update(req.body, {
        where: {
          id: req.params.itemId
        }
      })

      const response = await RefOrganization.findOne({
  ...
  async manyDestroy (req, res) {
    try {
      ...
      for (i = 0; i < req.body.length; i++) {
        await RefOrganization.destroy({ where: { id: req.body[i].id } })
      }
      res.send(req.body)
    } catch (err) {
      res.status(500).send({
  ...
  async manyDeActive (req, res) {
    try {
      for (var i = 0; i < req.body.length; i++) {
        await RefOrganization.update({ isActive: false }, { where: { id: req.body[i].id } })
      }
      res.send('success')
    } catch (err) {
      res.status(500).send({
  ...
  async manyRestore (req, res) {
    try {
      for (var i = 0; i < req.body.length; i++) {
        await RefOrganization.update({ isActive: true }, { where: { id: req.body[i].id } })
      }
      res.send('success')
    } catch (err) {
      res.status(500).send({
