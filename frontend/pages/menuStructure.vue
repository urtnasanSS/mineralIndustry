        ...
        <div v-for="item in items" :key="item.id">
          ...
          >
            <li @click="handleClickMore(item)">
              {{ $i18n.locale == 'mn' ? item.label : item.labelEn }}
            </li>
            <div v-for="i in item.children" :key="i.id">
              <div class="children1" @click="handleClickMore(i)">{{ $i18n.locale == 'mn' ? i.label : i.labelEn }}</div>
              <div v-if="i.children && i.children.length>0">
                <div v-for="unit in i.children" :key="unit.id">
                  <div class="children2" @click="handleClickMore(unit)">{{ $i18n.locale == 'mn' ? unit.label : unit.labelEn }}</div>
                  <div v-if="unit.children && unit.children.length>0">
                    <div v-for="value in unit.children" :key="value.id">
                      <div class="children3" @click="handleClickMore(value)">{{ $i18n.locale == 'mn' ? value.label : value.labelEn }}</div>
                    </div>
                  </div>
                </div>
              </div>
