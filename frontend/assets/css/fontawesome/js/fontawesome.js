...
;(function () {
  ...
  function getCanonicalIcon (values) {
    return values.reduce(function (acc, cls) {
      ...
      if (styles$1[cls]) {
        ...
        config.autoFetchSvg &&
        ['fas', 'far', 'fal', 'fad', 'fab', 'fa'].includes(cls)
      ) {
        acc.prefix = cls
      } else if (iconName) {
  ...
  function classParser (node) {
    ...
    const innerText = node.textContent !== undefined ? node.textContent.trim() : ''
    ...
    if (val.prefix && innerText.length > 1) {
      val.iconName = byLigature(val.prefix, node.textContent)
    } else if (val.prefix && innerText.length === 1) {
      val.iconName = byUnicode(val.prefix, toHex(node.textContent))
    }
